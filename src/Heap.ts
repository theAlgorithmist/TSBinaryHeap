...
export class TSMT$Heap
{
   ...
   */
  public extractRoot(): HeapData
  {
    if (this._data === undefined || this._data.length == 1)
      return null;

    let root: HeapData = this._data[1];
  ...
  public delete(value: number): void
  {
    ...
    if (index != 0)
    {
      ...
      const parent: number = Math.round(index/2);
  ...
  protected __heapifyDn(k: number): void
  {
    ...
    if (this._type == HeapType.MIN)
    {
      ...
      {
        let tmp: HeapData    = this._data[minIndex];
    ...
    else
    {
      ...
      {
        let tmp: HeapData    = this._data[maxIndex];
